...
def visualize_predicted_community_and_scatter(G, membership_pred_ranked, pos, K_true, palette, degree_info_file, output_prefix="dept_scatter"):
    ...
    nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color=edge_colors, alpha=edge_alpha, width=edge_width)
    plt.title(f"Predicted Community SBM (K={K_true})")
    plt.axis('off')
    plt.savefig("predicted_community_SBM.png", dpi=300, format="PNG")
    plt.close()
    ...
    for dept in sorted(dept_data.keys()):
        ...
        plt.title(f"SBM Classification: Department {dept}", fontsize=12)
...
def main():
    ...
    degree_info_file = "department_degree_info_SBM.txt"
    ...
    #visualize_predicted_community_and_scatter(G_filtered, membership_pred_ranked, pos, K_true, palette, degree_info_file, output_prefix="dept_scatter_SBM")
    ...
    with open('graph_info_SBM.txt', 'w') as f:
