...
def create_3x3_layout_ordered(G, membership, grid_width=20.0, grid_height=20.0, radius_divisor=2.0, seed=42):
    random.seed(seed)
    departments = sorted(list(set(membership.values())))
    if len(departments) != 9:
        raise ValueError("该布局函数要求部门数正好为9！(当前部门数={})".format(len(departments)))
    rows, cols = 3, 3
...
def visualize_predicted_community_and_scatter(G, membership_pred_ranked, pos, K_true, palette, degree_info_file, output_prefix="dept_scatter"):
    ...
    nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color=edge_colors, alpha=edge_alpha, width=edge_width)
    plt.title(f"Predicted Community BBM (K={K_true})")
    plt.axis('off')
    plt.savefig("predicted_community_BBM.png", dpi=300, format="PNG")
    plt.close()
    ...
    for dept in sorted(dept_data.keys()):
        ...
        plt.xlabel("Internal-degrees")
        plt.ylabel("External-degrees")
        plt.title(f"BBM Classification: Department {dept}", fontsize=12)
...
def main():
    ...
    Z_hat,gamma_hat,beta_hat = gbeta(A, K_true,512,16384)
    ...
    degree_info_file = "department_degree_info_BBM.txt"
    save_degree_info(G_filtered, membership_true_ranked, membership_pred_ranked, degree_info_file)
    pos = create_3x3_layout_ordered(G_filtered, membership_true_ranked, grid_width=20.0, grid_height=20.0, radius_divisor=2, seed=2)
    palette = sns.color_palette("Set1", K_true) if K_true == 9 else sns.color_palette("hls", K_true)
    #visualize_predicted_community_and_scatter(G_filtered, membership_pred_ranked, pos, K_true, palette, degree_info_file, output_prefix="dept_scatter_BBM")
    ...
    with open('graph_info_BBM.txt', 'w') as f:
