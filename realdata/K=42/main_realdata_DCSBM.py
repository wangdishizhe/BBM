...
def visualize_predicted_community_and_scatter(G, membership_pred_ranked, pos, K_true, palette, degree_info_file, output_prefix="dept_scatter"):
    ...
    for dept in sorted(dept_data.keys()):
        x = np.array(dept_data[dept][0])
        y = np.array(dept_data[dept][1])
        ...
        for color in unique_colors:
            ...
            plt.scatter(x_color, y_color, c=[color], alpha=1.0)
            
            if len(x_color) > 1000:
                model = LinearRegression(fit_intercept=False)
                model.fit(x_color.reshape(-1, 1), y_color)
                y_pred = model.predict(x_color.reshape(-1, 1))
                plt.plot(x_color, y_pred, color=color, linestyle='--', linewidth=1)
